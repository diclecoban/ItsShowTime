...
export function AuthPage({
  ...
  return (
    <ScrollView contentContainerStyle={[styles.authPage, isDesktop && styles.authPageDesktop]} showsVerticalScrollIndicator={false}>
      <View style={[styles.authIntro, isDesktop && styles.authIntroDesktop]}>
        <Text style={styles.authBrand}>It’s Showtime</Text>
        <Text style={[styles.authTitle, isDesktop && styles.authTitleDesktop]}>
          {mode === 'signin' ? 'Welcome back' : 'Create your watch home'}
        </Text>
        ...
        {isDesktop ? (
          <View style={styles.authDesktopStats}>
            <MiniStat label="Track" value="Shows" />
            <MiniStat label="React" value="Episodes" />
      ...
      <View style={styles.authFormColumn}>
        <View style={[styles.authCard, isDesktop && styles.authCardDesktop]}>
          ...
          {showValidation && !email.includes('@') ? <Text style={styles.authError}>Enter a valid email address.</Text> : null}
          <View style={styles.fakeInput}>
          ...
          {showValidation && password.length < 6 ? <Text style={styles.authError}>Password must be at least 6 characters.</Text> : null}
          ...
          {helperMessage ? <Text style={styles.authHelper}>{helperMessage}</Text> : null}
        </View>
